...
Show a table's schema and location:

```bash
faceberg user/mycatalog info stanfordnlp.imdb
```

Remove a table or an empty namespace:

```bash
faceberg user/mycatalog remove stanfordnlp.imdb
faceberg user/mycatalog remove stanfordnlp --yes
```
...
| Method | Description |
|--------|-------------|
| `init(config)` | Initialize the catalog storage, optionally with a `Config` |
...
| `drop_namespace(identifier)` | Remove an empty namespace from the catalog |
...
- [Buckets](buckets.qmd) — Store catalog metadata in HF Buckets (S3-like, no git history)
- [Architecture](design.qmd) — Understand how Faceberg works
- [DuckDB Integration](duckdb.qmd) — Advanced DuckDB queries
- [Pandas Integration](pandas.qmd) — Load data into DataFrames
