...
| Iceberg metadata | HuggingFace Spaces, Datasets, or Buckets |
| REST API | Auto-deployed Space (Buckets require running `faceberg serve` yourself) |

Spaces and Datasets store metadata with full git history; [Buckets](buckets.qmd)
are S3-like — no git history, no LFS — which makes them a lighter-weight option
for catalogs that don't need a hosted REST endpoint or revision history.
...
The Space exposes a read-only subset of the Iceberg REST catalog spec:
...
Writes (adding datasets, syncing, removing tables) happen through the `faceberg` CLI or Python API, not the REST server.
